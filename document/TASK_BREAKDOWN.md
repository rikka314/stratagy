...
> 整理时间：2026-03-20（Week 1 · Day 2/4 合并产出，Day 4 本地验证已补齐）
...
| 数据字段规范草案 | document/DATA_SCHEMA.md | 新建 | ✅ 完成 |
| 调研 AkShare A 股 API | — | 调研 | ✅ 完成（官方文档层面） |
| A 股数据本地验证 | — | 测试 | ✅ 完成（2026-03-20，本地实测） |
| 评估指标清单 | 写入本文件 | 文档 | ✅ 完成（附录 A） |
| UI 改版页面清单 | 写入本文件 | 文档 | ✅ 完成（附录 B） |
...
| 技术任务拆解表 | `document/TASK_BREAKDOWN.md` | Day 4 ✅ |
| 数据字段规范草案 | `document/DATA_SCHEMA.md` | Day 3 ✅ |
| 评估指标清单 | 本文件附录 A | Day 5 ✅ |
| UI 改版页面清单 | 本文件附录 B | Day 6 ✅ |

### A 股 API 本地验证记录（2026-03-20）

环境：Windows 本地 `.venv`，Python 3.13.7，`akshare 1.18.21`

测试方法：`ak.stock_zh_a_hist(symbol, period="daily", adjust="qfq")`

| 代码 | 行数 | 日期范围 | 耗时 | 观察 |
|------|------|---------|------|------|
| `000001` | 8358 | 1991-04-03 ~ 2026-03-20 | 0.677s | 字段稳定，覆盖全历史 |
| `600519` | 5883 | 2001-08-27 ~ 2026-03-20 | 0.662s | 字段稳定，行数随上市时间减少 |
| `300750` | 1886 | 2018-06-11 ~ 2026-03-20 | 0.537s | 字段稳定，创业板样本正常 |

结论：

- 接口在本地环境可稳定返回 12 列：`日期/股票代码/开盘/收盘/最高/最低/成交量/成交额/振幅/涨跌幅/涨跌额/换手率`
- 不同股票的数据量差异主要来自上市时间，不影响字段映射方案
- `adjust="qfq"` 的极早期数据可能出现负价格或极端百分比；W2 接入时应将其作为已知风险记录，避免直接把全历史前复权值用于展示结论
...
- [x] AkShare A 股 API 已本地验证（W1 Day 4）
...
| 任务 | 涉及文件 | 改动详情 |
|------|---------|---------|
| 统一评估输出格式 | core/backtest.py | 微调 `simulate_strategy` 返回值，确保 evaluation.py 可直接消费，并为后续多模型比较保留统一结果结构 |
| 新建评估模块 | **core/evaluation.py** | 第一批指标：累计收益、年化收益、最大回撤、夏普、胜率、盈亏比、换手率；基准对比；统一 comparison schema |
| 导出到 __init__.py | core/__init__.py | 加 evaluation 导出 |
| 评估区块嵌入 | ui/single_stock.py | 新增评估区：净值曲线、回撤曲线、统计表；为后续原策略/模型同台竞技保留 compare tabs |
...
- [ ] 单股页面能展示完整评估区块
- [ ] 策略 vs 买入持有对比表
- [ ] evaluation 输出结构可被后续 ML 模型比较直接复用
...
| ML 开关 + 结果展示 | ui/single_stock.py | ML 开关按钮 + 原策略/Logistic/LightGBM/买入持有 单股对比 + 特征重要性图 |
...
| 对比展示增强 | ui/single_stock.py | 多区间表现对比 + 模型质量指标卡（Precision/Recall/F1/PR-AUC/通过率） |
...
| 任务 | 涉及文件 | 改动详情 |
|------|---------|---------|
| 评估增强 | core/evaluation.py | 第二批指标：Calmar、Sortino、平均持有天数、最大连续亏损；分区间表现、参数对比、多股票汇总口径 |
| 图表风格统一 | core/visualization.py | 配色、字体、边距、图例位置；新增股票 × 模型热图、模型箱线图、跑赢基准占比图 |
| UI 打磨 | single_stock.py, multi_stock.py | 空状态提示、加载动画、配色统一；补多股票模型汇总看板 |
| 文档 | README.md + 使用说明 | — |
...
| 指标 | 用途 | 备注 |
|------|------|------|
| Precision | 看“模型放行的信号里有多少是真的好信号” | 适合信号过滤场景 |
| Recall | 看“好信号里有多少被模型保留下来” | 防止模型过度保守 |
| F1 | 平衡 Precision / Recall | 适合二分类基线 |
| PR-AUC | 类别不平衡时比 ROC-AUC 更有用 | 推荐主指标之一 |
| Brier score / 校准曲线 | 看预测概率是否可信 | 后续用于概率分层仓位 |
| 信号通过率 | 看模型是否靠“几乎不交易”取得好成绩 | 必须和交易结果一起看 |

### 评估框架分层

- **交易结果层**：收益、回撤、夏普、胜率、盈亏比、换手率、样本外表现
- **模型质量层**：Precision、Recall、F1、PR-AUC、校准、信号通过率
- **展示维度层**：单股页看细节，多股页看泛化
...
- 策略 vs 买入持有
- 原策略 vs ML 增强
- Logistic vs LightGBM
- 不同参数组对比
- 单个股票表现 vs 多股票汇总表现

### 建议核心图表

- 单股多模型净值折线图：原策略 / Logistic / LightGBM / 买入持有
- 单股多模型回撤对比图
- 单股滚动超额收益曲线
- 多股票 `股票 × 模型` 热力图
- 多股票模型箱线图
...
### 现有代码覆盖情况（基于 `core/backtest.py`）

| 项目 | 当前状态 | 说明 |
|------|---------|------|
| 最大回撤 | 已有 | `max_drawdown(equity)` 已实现 |
| 夏普比率 | 已有 | `sharpe_ratio(returns)` 已实现 |
| 累计收益 | 可直接计算 | `simulate_strategy()` 已返回 `strategy_equity` |
| 年化收益 | 可直接计算 | 基于 `strategy_equity` 与交易天数计算 |
| 策略 vs 买入持有对比 | 可直接计算 | `simulate_strategy()` 已返回 `buy_hold_equity` |
| 换手率 | 需在评估层补算 | 可基于 `position` 变化次数 / 总天数推导 |
| 胜率 | 缺失 | 当前未输出逐笔交易结果 |
| 盈亏比 | 缺失 | 当前未输出逐笔交易结果 |
| Calmar 比率 | 缺失 | 需在 `core/evaluation.py` 统一实现 |
| Sortino 比率 | 缺失 | 需在 `core/evaluation.py` 统一实现 |
| 平均持有天数 | 缺失 | 需要 trade-level 重建或额外输出 |
| 最大连续亏损次数 | 缺失 | 需要 trade-level 重建或额外输出 |
| 模型质量指标 | 缺失 | 需由 `core/ml_filter.py` 输出预测结果，再由 `core/evaluation.py` 汇总 |
| 多模型比较结构 | 缺失 | 需统一 `model_name/scope/split/metrics` 结果 schema |

### 对 W3 评估模块实现的直接约束

- `core/evaluation.py` 不只是封装公式，还要统一消费 `strategy_equity`、`strategy_return`、`buy_hold_equity`
- 逐笔型指标（胜率、盈亏比、平均持有天数、最大连续亏损次数）不能只靠当前汇总列稳妥得出
- W3 更稳妥的实现方式是：让 `simulate_strategy()` 额外输出 `entry_flag` / `exit_flag` 或单独 trade log，再由 `core/evaluation.py` 汇总
- `core/evaluation.py` 需要支持统一比较结构，至少包含 `name/scope/split/trade_metrics/model_metrics`
- 单股页与多股页应复用同一比较结构，避免后续图表口径不一致
...
├── 模型同台竞技区（W5）：原策略 / Logistic / LightGBM / 买入持有
├── 模型质量区（W5/W7）：Precision / Recall / F1 / PR-AUC / 校准 / 特征重要性
└── 导出区
...
├── 相关性热图
├── 风险收益散点
├── 模型汇总评估（W7）：股票 × 模型热力图、模型箱线图、跑赢基准占比
└── 组合模拟
```
...
- 单股页优先展示时序细节，不要把多股汇总图塞进同一视图
- 多股页优先展示泛化和稳定性，不重复单股细节图
- 模型质量指标不能脱离交易结果单独展示
