...
| 动态专家组合研究 | `model-test/model_test/moe_baseline.py`、`model-test/model_test/expert_panel.py`、`model-test/model_test/dynamic_ensemble.py`、`model-test/model_test/dynamic_ensemble_v2.py`、`model-test/model_test/dynamic_ensemble_online.py`、`model-test/model_test/dynamic_ensemble_bandit.py`、`model-test/prepare_moe_baseline.py`、`model-test/prepare_expert_panel.py`、`model-test/run_dynamic_ensemble.py`、`model-test/run_dynamic_ensemble_v2.py`、`model-test/run_dynamic_ensemble_online.py`、`model-test/run_dynamic_ensemble_bandit.py`、`model-test/configs/moe_baseline_*.json`、`model-test/configs/moe_v[12]_*.json`、`model-test/configs/moe_online_*.json`、`model-test/configs/moe_bandit_*.json` | 冻结 Window 3B / full-run 输出、逐日净收益 artifact、无泄漏专家面板与 ready Phase-C/D/E source lock | Phase-A 三项对照、Phase-B `expert_day_panel` / purged splits、Phase-C soft gate、Phase-D quantile gate / 降级 / 风险约束 / 消融 / v1-v2 对照、Phase-E Hedge/EG 完整反馈回放、Phase-F selected-action-only LinUCB / contextual Thompson Sampling、漂移响应与 source-lock 对照 | `document/interfaces/dynamic-ensemble-research.md` | `strategy-core-pipeline` |
